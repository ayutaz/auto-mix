...
class TestLimiterProcessor:
    ...
    def test_lookahead_limiting(self):
        """ルックアヘッドリミッティングのテスト"""
        sr = 44100
        limiter_no_lookahead = LimiterProcessor(
            sample_rate=sr,
            threshold_db=-6,
            lookahead_ms=0
        )
        
        limiter_with_lookahead = LimiterProcessor(
            sample_rate=sr,
            threshold_db=-6,
            lookahead_ms=10
        )
...
class TestMultibandCompressor:
    ...
    def test_frequency_balance(self):
        ...
        correlation = np.corrcoef(
            np.log10(fft_original + 1e-10),
            np.log10(fft_compressed + 1e-10)
        )[0, 1]
...
class TestLoudnessNormalizer:
    ...
    def test_streaming_platforms_presets(self):
        ...
        for platform, target_lufs in platforms.items():
            normalizer = LoudnessNormalizer.from_preset(
                platform,
                sample_rate=sr
            )
