...
class TestMasteringProcessorEdgeCases:
    ...
    def test_custom_settings(self):
        ...
        settings = MasteringSettings(
            use_eq=True, use_multiband_comp=False, use_limiter=False, eq_preset="bright"
        )
...
class TestIntegrationMastering:
    ...
    def test_full_mastering_pipeline(self):
        ...
        for i in range(0, len(t), sr // 2):  # 120 BPM
            kick_pattern[i : i + 1000] = np.exp(-np.linspace(0, 1, 1000) * 10) * np.sin(
                2 * np.pi * 60 * np.linspace(0, 1, 1000)
            )
        ...
        # リード
        lead = (
            0.2 * np.sin(2 * np.pi * 440 * t) * (1 + 0.3 * np.sin(2 * np.pi * 5 * t))
        )  # ビブラート
        ...
        low_ratio_mastered = np.mean(fft_mastered[:low_end]) / np.mean(
            fft_mastered[low_end:mid_end]
        )
    ...
    def test_genre_specific_mastering(self):
        ...
        edm_settings = MasteringSettings(
            target_lufs=-9, ceiling_db=-0.1, use_multiband_comp=True, eq_preset="bright"
        )

        # ジャズ向け設定（ダイナミック、暖かい）
        jazz_settings = MasteringSettings(
            target_lufs=-16, ceiling_db=-1.0, use_multiband_comp=False, eq_preset="warm"
        )
