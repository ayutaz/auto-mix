...
class SpectrumVisualizer:
    ...
    def render_frame(self, audio_data: NDArray[np.float32]) -> NDArray[np.uint8]:
        ...
        # スムージング
        spectrum = (self.smoothing_factor * self.prev_spectrum +
    ...
    def _draw_bars(self, frame: NDArray[np.uint8], spectrum: NDArray[np.float32]) -> None:
        ...
        for i in range(self.bar_count):
            ...
            cv2.rectangle(
                frame,
                (x, y),
                (x + bar_width - bar_spacing * 2, self.height),
                color,
                -1
            )
...
class ParticleVisualizer:
    ...
    def render_frame(
        self,
        audio_level: float,
        frequency_data: Optional[NDArray[np.float32]] = None
    ) -> NDArray[np.uint8]:
        """
        1フレーム分のパーティクルを描画
    ...
    def _spawn_particles(
        self,
        audio_level: float,
        frequency_data: Optional[NDArray[np.float32]]
    ) -> None:
        """新しいパーティクルを生成"""
