...
class ChorusProcessor:
    ...
    def _variable_delay(
        self,
        audio: NDArray[np.float32],
        delay_samples: NDArray[np.float32]
    ) -> NDArray[np.float32]:
        """可変ディレイの適用"""
        output = np.zeros_like(audio)
...
class DeesserProcessor:
    ...
    def process(self, audio: NDArray[np.float32]) -> NDArray[np.float32]:
        ...
        sos = scipy.signal.butter(
            4, self.frequency, btype='highpass',
...
class StereoProcessor:
    ...
    def to_mid_side(self, stereo: NDArray[np.float32]) -> tuple[NDArray[np.float32], NDArray[np.float32]]:
        """L/R to Mid/Side変換"""
