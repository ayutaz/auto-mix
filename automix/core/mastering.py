...
class LimiterProcessor:
    ...
    def __init__(
        ...
        self.delay_buffer: Optional[NDArray[np.float32]] = None
    ...
    def process(self, audio: NDArray[np.float32]) -> NDArray[np.float32]:
        ...
        for i in range(len(audio)):
            # ルックアヘッド区間の最大値
            lookahead_max = np.max(np.abs(
                padded_audio[i:i + self.lookahead_samples]
            ))
...
class MultibandCompressor:
    ...
    def __init__(
        ...
        self.ratios = ratios
        self.attack_coefs = [
            np.exp(-1 / (sample_rate * ms / 1000)) for ms in attack_ms
        ]
        self.release_coefs = [
            np.exp(-1 / (sample_rate * ms / 1000)) for ms in release_ms
        ]
        ...
        for i in range(self.num_bands):
            if i == 0:
                ...
                sos = scipy.signal.butter(
                    4, self.crossover_frequencies[0],
                    btype='lowpass', fs=self.sample_rate, output='sos'
                )
            elif i == self.num_bands - 1:
                # 最高帯域（ハイパス）
                sos = scipy.signal.butter(
                    4, self.crossover_frequencies[-1],
                    btype='highpass', fs=self.sample_rate, output='sos'
                )
            else:
                # 中間帯域（バンドパス）
                sos = scipy.signal.butter(
                    4, [self.crossover_frequencies[i-1], self.crossover_frequencies[i]],
...
class LoudnessNormalizer:
    ...
    def measure_integrated_lufs(self, audio: NDArray[np.float32]) -> float:
        ...
        # High-pass at 38 Hz
        sos2 = scipy.signal.butter(
            2, 38, btype='highpass', fs=self.sample_rate, output='sos'
        )
...
class FinalEQ:
    ...
    def process(self, audio: NDArray[np.float32], preset: Optional[str] = None) -> NDArray[np.float32]:
        """
        EQ処理を適用
