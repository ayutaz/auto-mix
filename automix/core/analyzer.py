...
class TempoAnalysis:
    ...
    downbeats: NDArray[np.float32] | None = None
...
class AudioAnalyzer:
    ...
    def analyze_pitch(
        self, audio: NDArray[np.float32], fmin: float = 80.0, fmax: float = 2000.0
    ) -> PitchAnalysis:
        """
        ピッチを解析する
        ...
        pitches, voiced_flag, voiced_probs = librosa.pyin(
            audio, fmin=fmin, fmax=fmax, sr=self.sample_rate, hop_length=self.hop_length
        )
    ...
    def analyze_volume(self, audio: NDArray[np.float32]) -> VolumeAnalysis:
        ...
        peaks, _ = scipy.signal.find_peaks(
            envelope, height=peak * 0.7, distance=self.sample_rate // 10
        )
    ...
    def analyze_spectrum(self, audio: NDArray[np.float32]) -> SpectralAnalysis:
        ...
        # ゼロ交差率
        zero_crossing_rates = librosa.feature.zero_crossing_rate(audio, hop_length=self.hop_length)
